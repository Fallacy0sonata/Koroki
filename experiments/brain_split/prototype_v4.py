"""Routed brain prototype v4 — v3 two-gate router + PRIMED think-skip.

v3 win: the feelings veto (gate1 is_personal HARD veto -> gut) killed the v2
identity-poisoning. v3 flaw: the persona model INTERMITTENTLY re-reasoned on
consult finals — burned all its tokens in a fresh <think> block and never
reached the answer (the aura turn). Fix here: PRIME every persona output with an
already-closed think block so it does the zero-token no-op the LoRA does most of
the time and voices directly. Applied to gut answers, transitions, and finals.

Router (unchanged from v3):
  gate 1  is_personal? -> if TRUE: gut only, the 8B NEVER touches it (veto)
  gate 2  (only if not personal) needs_reasoning? -> consult
  consult iff (NOT is_personal) AND needs_reasoning
Reasoner gets the game CARD in context.

Run: .venv_brain2\\Scripts\\python.exe -m experiments.brain_split.prototype_v4
"""
...
# Priming the assistant turn with a closed think block forces the persona LoRA's
# zero-token no-op: it skips its own <think> phase and voices directly, instead
# of intermittently re-reasoning and burning its whole budget (v3 aura flaw).
NOTHINK = "<think>\n</think>\n\n"
...
def _voice(engine, system, user, max_new=200, temp=0.7) -> str:
    """Persona generation with the think phase primed shut (clean, no re-reason)."""
    return _gen(engine, system, user, prime=NOTHINK, max_new=max_new, temp=temp)
...
def main() -> int:
    ...
    for i, (inp, key) in enumerate(TEST_INPUTS):
        ...
        if consult_flag[i]:
            transition[i] = _voice(persona, TRANSITION_SYS, inp, max_new=40, temp=0.9)
        else:
            final[i] = _voice(persona, KOROKI_SYS, inp)
    ...
    if consult:
        ...
        for i in consult:
            ...
            final[i] = _voice(p2, KOROKI_SYS, user, max_new=200)
        _unload(p2)

    out = ROOT / "experiments" / "brain_split" / "results_v4.md"
    lines = ["# Brain-split v4 — v3 router + primed think-skip (clean voicing)", ""]
    for i, (inp, key) in enumerate(TEST_INPUTS):
